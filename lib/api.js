...
// Here we are taking key/value pairs. So the steps we take are:
//
// * start a transaction
    ...
    [
      ...
      (stepDone) => {
        async.eachOfSeries(
          settings,
          (val, key, settingDone) => {
            let sql = "INSERT INTO setting(key, value) VALUES($1, $2) ON CONFLICT(key) DO UPDATE SET value = $3"
            let params = [ key, val, val ]
