...
function insEvent(id, username, payload, callback) {
  let query = {
    text   : `
      INSERT INTO
    ...
    `,
    values : [ id, username, JSON.stringify(payload) ],
  }
  db.exec(null, query, callback)
}

function insEventStripe(payload, callback) {
  ...
  insEvent(payload.id, 'stripe', payload, callback)
}
