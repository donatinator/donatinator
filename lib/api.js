...
function selPlansAll(callback) {
  const sql = `
    SELECT
      plan.*,
      interval.name AS interval,
      interval.position
    FROM
      plan
      LEFT JOIN interval ON (plan.interval_id = interval.id)
    ORDER BY
      position,
      amount
  `
...
function getPlan(name, callback) {
  const sql = `
    SELECT
      plan.*,
      interval.name AS interval
...
function selIntervalsAll(callback) {
  db.getAllArr(null, "SELECT * FROM interval ORDER BY position", callback)
}

function insPlan(plan, callback) {
  var query = {
    text   : `
      INSERT INTO
        plan(name, title, description, amount, interval_id, currency)
      VALUES(
        $1,
        $2,
        $3,
        $4,
        (SELECT id FROM interval WHERE name = $5),
        (SELECT value FROM setting WHERE name = 'currency')
      )
    `,
    values : [ plan.name, plan.title, plan.description, plan.amount, plan.interval ],
  }
