...
function loadMenu(req, res, next) {
  // if we already have the menu
  if ( menu ) {
    res.locals.menu = menu
    return next()
  }

  // no menu yet, so (force) reload them
  reloadMenu(req, res, next)
}

function reloadMenu(req, res, next) {
  // we don't check if the menu is already loaded here, just reload it from the db
  api.selPagesAllMenu(null, (err, pages) => {
    if (err) {
      console.warn(err)
      return next(err)
    }

    // pages loaded fine, so turn them into an array with just the name, title, and position
    let newMenu = pages.map(page => ({
      name     : page.name,
      title    : page.title,
...
function reloadGifts(req, res, next) {
  ...
  api.selGiftsAll(null, (err, result) => {
    if (err) {
      console.warn(err)
      return next(err)
    }
