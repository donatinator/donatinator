...
app.post('/admin/page/new', (req, res, next) => {
  ...
  // we shouldn't ever have an 'id' here
  delete form.id
  ...
  if ( false ) {
    // ToDo: fix this with the validation stuff above
    res.render('admin-page-form', {
      form   : form,
      errors : {
        title : 'Must be something',
        amount : 'Must be something else',
      }, // no errors yet!
      intervals : intervals,
    })
    return
...
app.post('/admin/page/:name/edit', (req, res, next) => {
  ...
  // ToDo: validate the incoming params
  if ( false ) {
...
app.get('/admin/plan/new', (req, res, next) => {
  ...
  api.selIntervalsAll((err, intervals) => {
    if (err) return next(err)
...
app.post('/admin/plan/new', (req, res, next) => {
  ...
  // insert this plan into the database
  api.insPlan(plan, (err) => {
    if (err) return next(err)
