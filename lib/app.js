...
app.post('/hook/stripe', rawBodyParser, stripeMiddleware.webhook, (req, res, next) => {
  ...
  console.log('/hook/stripe - event.id=' + event.id)

  // ToDo: put this into the database
  api.insEventStripe(event, (err) => {
    if (err) return next(err)
